...
from users.models import CustomUser, UserTvMediaRating, UserTvMediaGenrePreference,UserBookRating
...
class UserTvMediaRatingTestCase(TestCase):
    ...
    def setUp(self):
        # Create test user
        self.user = CustomUser.objects.create_user(email='test@example.com', password='password123', first_name='Test', last_name='User')
    ...
    def test_update_media_genre_preferences(self):
        # Initial rating
        rating = UserTvMediaRating.objects.create(user=self.user, tvmedia=self.tvmedia1, rating=8)
        ...
        # Add another rating
        rating2 = UserTvMediaRating.objects.create(user=self.user, tvmedia=self.tvmedia2, rating=6)
    ...
    def test_cache_invalidation_on_rating_save(self):
        ...
        rating = UserTvMediaRating.objects.create(user=self.user, tvmedia=self.tvmedia1, rating=7)
    ...
    def test_signal_triggered_on_rating_save(self):
        ...
        rating = UserTvMediaRating.objects.create(user=self.user, tvmedia=self.tvmedia1, rating=9)
...
class UserTestCase(TestCase):
    def setUp(self):
        self.user = CustomUser.objects.create_user(email='test@example.com', password='password', first_name='Test')
    ...
    def test_invalid_rating(self):
        with self.assertRaises(ValidationError):
            UserTvMediaRating.objects.create(user=self.user, tvmedia=self.tvmedia, rating=11).full_clean()

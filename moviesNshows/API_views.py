            ...
            return Response(
                {
                    "data": self.serializer(
                        self.model.objects.order_by("-startyear")[:50], many=True
                    ).data
                }
            )
...
class PublicRecommendTvMedia(APIView):
    ...
    def post(self, request):
        needed = request.data
        try:
            needed: dict[Genre, int] = {
                Genre.objects.get(name=key): i for key, i in needed.items()
            }
        ...
        data = {}
        for ind, media in enumerate(
            TvMediaSerializer(final_sort, many=len(sort) > 1).data
        ):
...
class PrivateRecommendTvMedia(APIView):
    ...
    def get(self, request):
        ...
        if not data:
            ...
            data = {}
            for ind, media in enumerate(
                self.serializer(final_sort, many=len(sort) > 1).data
            ):
                data[str(ind)] = {"relativity": sort[ind][0], "media": media}

...
def get_item_similarities(item_id: Any, interaction_model: Type[Model], item_field: str) -> List[Tuple[float, Any]]:
    """
    ...
    users_who_rated = interaction_model.objects.filter(**{item_field: item_id}).values_list('user_id', flat=True)
    
    # Get all ratings for these users (to build item profiles)
    related_ratings = interaction_model.objects.filter(user_id__in=users_who_rated).select_related(item_field)
...
def get_collaborative_recommendations(user: Any, interaction_model: Type[Model], item_model: Type[Model], item_field: str, top_n: int = 10) -> List[Tuple[float, Any]]:
    """
    ...
    user_interactions = interaction_model.objects.filter(user=user, rating__gte=7).values_list(item_field, 'rating')
    ...
    recommendations = []
    already_rated = set(interaction_model.objects.filter(user=user).values_list(item_field, flat=True))

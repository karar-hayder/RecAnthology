...
from myutils.collaborative_filtering import calculate_cosine_similarity, get_item_similarities, get_collaborative_recommendations
from myutils.recommendation import get_hybrid_recommendation

class CollaborativeFilteringTests(TestCase):
    def setUp(self):
        self.user1 = CustomUser.objects.create_user(email="user9991@example.com", password="password", first_name="User1")
        self.user2 = CustomUser.objects.create_user(email="user9992@example.com", password="password", first_name="User2")
        self.user3 = CustomUser.objects.create_user(email="user9993@example.com", password="password", first_name="User3")
        
        self.genre1 = Genre.objects.create(name="Genre1")
        
        self.book1 = Book.objects.create(title="Book1", author="Author1", isbn="1", pages=100, likedPercent=90)
        self.book1.genre.add(self.genre1)
        self.book2 = Book.objects.create(title="Book2", author="Author2", isbn="2", pages=100, likedPercent=90)
        self.book2.genre.add(self.genre1)
        self.book3 = Book.objects.create(title="Book3", author="Author3", isbn="3", pages=100, likedPercent=90)
    ...
    def test_collaborative_recommendations(self):
        ...
        recs = get_collaborative_recommendations(self.user3, UserBookRating, Book, 'book')
        print(recs)
